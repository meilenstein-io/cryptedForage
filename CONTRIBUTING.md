...
Please
[check the existing issues](https://github.com/mozilla/cryptedForage/issues) to
...
See below for [issues that have been discussed and will be rejected](#features-cryptedforage-will-reject).

# Contributing to cryptedForage

First off: thanks! Open source software (and thus all software) exists because
of people like you. <3

If you'd like to contribute to cryptedForage, it's as simple as opening a pull
...
* **all drivers must have the same public API**: refer to [the API tests](https://github.com/mozilla/cryptedForage/blob/master/test/test.api.coffee) for how we ensure this, but simply put: all drivers should share the _exact same_ outward-facing API
...
built versions of `cryptedforage.js` and `cryptedforage.min.js`, which are
used by bower.
...
## Features cryptedForage will reject

### node.js support

cryptedForage is a browser library with a specific focus on client-side,
...
cryptedForage API wouldn't be hard (it's just localStorage with callbacks and
